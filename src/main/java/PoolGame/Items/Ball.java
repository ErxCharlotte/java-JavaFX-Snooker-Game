...
public class Ball implements Drawable, Movable {
    ...
    private ScoreStrategy scoreAction;

    private boolean isStoreMemento = false;
        ...
        this.shape.setOnMouseClicked(
                (actionEvent) -> {
                    if (this.hasStopped()) {
                        ...
                        this.mouseDragLine.setStrokeWidth(11);
                        this.mouseDragLine.setStartX(this.shape.getCenterX());
                        this.mouseDragLine.setStartY(this.shape.getCenterY());
                        this.mouseDragLine.setEndX(this.shape.getCenterX() + 60);
                        this.mouseDragLine.setEndY(this.shape.getCenterY() + 60);
                    }
                }
        );

        this.mouseDragLine.setOnMouseDragged(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    this.mouseDragLine.setStartX(actionEvent.getSceneX());
                    this.mouseDragLine.setStartY(actionEvent.getSceneY());

                    this.mouseDragLine.setEndX((this.shape.getCenterX()+actionEvent.getSceneX())/2);
                    this.mouseDragLine.setEndY((this.shape.getCenterY()+actionEvent.getSceneY())/2);
                    this.isStoreMemento = false;
                }
            }
        );

        this.mouseDragLine.setOnMouseReleased(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    this.mouseDragLine.setVisible(false);
    ...
    */
    ...
    public void fallIntoPocket(Game game) {
        ...
        game.addScore(this.scoreAction.getScore(game, this));
    }
    ...
    public ScoreStrategy getScoreAction() {
        return scoreAction;
    }

    public boolean isStoreMemento() {
        return isStoreMemento;
    }

    public void setIsStoreMemento(boolean is){
        this.isStoreMemento = is;
    }
